package com.view.simpleratingbar;
...
public class SimpleRatingBar extends View {
  ...
  public enum Gravity {
    ...
    static Gravity fromId(int id) {
      ...
      // default value
      Utils.printLog("SimpleRatingBar", String.format("Gravity chosen is neither 'left' nor 'right', I will set it to Left"));
      return Left;
    }
  }
  ...
  private void parseAttrs(AttributeSet attrs) {
    ...
    borderColor = arr.getColor(R.styleable.SimpleRatingBar_SRB_borderColor, getResources().getColor(android.R.color.transparent));
  ...
  private void validateAttrs() {
    ...
    if (desiredStarSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && desiredStarSize
        > maxStarSize) {
      Utils.printLog("SimpleRatingBar", String.format("Initialized with conflicting values: starSize is greater than maxStarSize (%f > %f). I will ignore maxStarSize", desiredStarSize, maxStarSize));
    }
    if (stepSize <= 0) {
  ...
  private void setNewRatingFromTouch(float x, float y) {
    ...
    float mod = rating % 0.5f;
    if (mod < 0.5f/4) {
      rating = rating - mod;
      rating = Math.max(0, rating);
    } else {
      rating =  rating - mod + 0.5f;
...
//      }
  ...
  public void setStarSize(float starSize) {
    ...
    if (starSize != Integer.MAX_VALUE && maxStarSize != Integer.MAX_VALUE && starSize > maxStarSize) {
      Utils.printLog("SimpleRatingBar", String.format("Initialized with conflicting values: starSize is greater than maxStarSize (%f > %f). I will ignore maxStarSize", starSize, maxStarSize));
    }
    // force re-calculating the layout dimension
    requestLayout();
    generateInternalCanvas(getWidth(), getHeight());
  ...
  private float normalizeRating(float rating) {
    if (rating < 0) {
      Utils.printLog("SimpleRatingBar", String.format("Assigned rating is less than 0 (%f < 0), I will set it to exactly 0", rating));
      return 0;
    } else if (rating > numberOfStars) {
      Utils.printLog("SimpleRatingBar", String.format("Assigned rating is greater than numberOfStars (%f > %d), I will set it to exactly numberOfStars", rating, numberOfStars));
      return numberOfStars;
    } else {
      return rating;
    }
